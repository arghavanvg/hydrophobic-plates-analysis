...
n_slices = int((y_max - y_min) / slice_thickness)
y_starts = np.array([round(y_min + i * slice_thickness, 3) for i in range(n_slices)])
y_ends = y_starts + slice_thickness
y_distances = [round((i + 1) * slice_thickness * 10, 1) for i in range(len(y_starts))]

y_counts = [[] for _ in range(len(y_starts))]

# Loop through frames
for frame_num in range(n_frames):
    coords = traj.xyz[frame_num][all_ox_tpl_indices]

    for i, (y_start, y_end) in enumerate(zip(y_starts, y_ends)):
        mask = (
            (coords[:, 0] >= x_min) & (coords[:, 0] < x_max) &
            (coords[:, 1] >= y_start) & (coords[:, 1] < y_end) &
            (coords[:, 2] >= z_min) & (coords[:, 2] < z_max)
        )
        y_counts[i].append(np.sum(mask))

y_data = [[float(y_dist), float(np.mean(counts))] for y_dist, counts in zip(y_distances, y_counts)]
np.savetxt(f"{output_path}counts-y.dat", y_data, fmt="%.1f\t%.8f")

v = float_d * 10 * slice_thickness * 10 * 17.62 #A^3
numdens_data = [[float(y_dist), float(np.mean(counts))/v] for y_dist, counts in zip(y_distances, y_counts)]
np.savetxt(f"{output_path}numdens-y.dat", numdens_data, fmt="%.1f\t%.5f")


x_axis = [float(y_dist) for y_dist, _ in y_data]
no_wats = [float(count) for _, count in y_data]
...
plt.title("Number Density Distribution Along Y-axis")
plt.xlabel('Distance from the edge in Å', fontsize=14)
plt.ylabel('Number density referenced to bulk density', fontsize=14)
plt.legend()
plt.xlim(0, 18)
plt.ylim(bottom=0)
# plt.xticks([-4.0, -2.0, 0.0, 2.0])
